...
import * as mockNavJson from '../../__mocks__/data/zuza-navigation-response.json';
import * as mockArticlesJson from '../../__mocks__/data/zuza-articles-response.json';
import * as mockPublicationJson from '../../__mocks__/data/thestar-publication-response.json';
...
describe('transforms', () => {
  describe('transformNavigation()', () => {
    ...
    it('should set PrimaryNavItem.children = navigationItems if defined', () => {
      const results = t.transformNavigation(mockNavJson);
      expect(results[0].children.length).toEqual(
        mockNavJson.items[0].navigationItems.length
      );
    });
  });
  ...
  describe('transformArticleItems()', () => {
    it('should return empty array if an error occurs', () => {
      ...
      const results = t.transformArticleItems({ assetId: '1' } as any);
      expect(results).toEqual([]);
    });
    it('should return empty array if items is not defined', () => {
    ...
    it('should return an array of valid NewsItems', () => {
      ...
      results.forEach((item, index) => {
        ...
        expect(item.displayPublishDate).toEqual(
          items[index].displayPublishFromDate
        );
